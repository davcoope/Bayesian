...
class BO:
    def __init__(self, KernelFunction, length_scale, AcquisitionFunction, bounds, acquisition_samples, log_path=None, dynamic_bounds=False, iterations_between_reducing_bounds=None, first_reduce_bounds=None, reduce_bounds_factor=None, random_seed=42, minimize=False):
        """
        ...
        - acquisition_samples (int): Number of samples to take. These are the candidate points. 
        ...
        if self.X_data.size == 0:
            ...
            if sub_batch_size is None:
                ...
                for i in range(batch_size):
                    if kappa == 'default':
                        raw_X[i] = self.GetNextX(K_inv=K_inv).flatten()
                        ...
                        raw_X[i] = self.GetNextX(kappa, K_inv=K_inv).flatten()
    ...
    def StuckInPeak(self):
        ...
        if self.minimize is False:
            ...
            # Find the smalles index from the most recent iteration
        ...
        if (self.minimize is False and best_value[0] > self.current_best_value) or (self.minimize is True and best_value[0] < self.current_best_value):
            ...
            self.current_best_value = best_value[0]  # Update the current best value
        else:
